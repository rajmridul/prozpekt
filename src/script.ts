...
interface YouTubePlaylistItemResourceId {
    kind: string;
    videoId: string;
}
...
function loadConfig(): Config {
    return {
        youtubeApiKey: 'YOUR_API_KEY_HERE' // This will be replaced when building
    };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const introContainer = document.querySelector('.intro-container') as HTMLDivElement;
    const playerWrapper = document.getElementById('playerWrapper') as HTMLDivElement;
    ...
    const albumName = document.getElementById('albumName') as HTMLParagraphElement;
    ...
    const albumListEl = document.getElementById('albumList') as HTMLUListElement;
    const currentAlbumTitle = document.getElementById('currentAlbumTitle') as HTMLHeadingElement;
    const listenerCountEl = document.getElementById('listenerCount') as HTMLSpanElement;
    const socialListenerCountEl = document.getElementById('socialListenerCount') as HTMLSpanElement;
    const albumArtContainer = document.getElementById('albumArtContainer') as HTMLDivElement;
    ...
    let playlistData: Track[] = []; // Will hold the YouTube playlist data

    // Create YouTube iframe player
    function createYouTubePlayer(): HTMLIFrameElement {
    ...
    // Get unique albums
    function getUniqueAlbums(): string[] {
    ...
    function updateListenerCount(): void {
        fakeListeners += Math.floor(Math.random() * 3) - 1;
        fakeListeners = Math.max(10, Math.min(100, fakeListeners));
        if (listenerCountEl) listenerCountEl.textContent = fakeListeners.toString();
        if (socialListenerCountEl) socialListenerCountEl.textContent = fakeListeners.toString();
    }
